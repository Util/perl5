 ...
 *    XXX manual override in Configure.
 *
 ...
 *      LC_CTYPE locale is toggled.  To save work that would be discarded, the
 *      bool_setlocale_X() implementations don't do this extra work.  Instead,
 *      there is a separate function for just this purpose to be done before
 *      control is transferred back to the external caller.  All categories
 *      that have such requirements have such a function.  The
...
#    define LC_CTYPE_ADD_LEN(extra)    (STRLENs("LC_CTYPE") + extra)
...
#    define LC_TIME_ADD_LEN(extra)      (STRLENs("LC_TIME") + extra)
...
#    define LC_MESSAGES_ADD_LEN(extra)    (STRLENs("LC_MESSAGES") + extra)
...
#    define LC_MONETARY_ADD_LEN(extra)    (STRLENs("LC_MONETARY") + extra)
...
#    define LC_ADDRESS_ADD_LEN(extra)   (STRLENs("LC_ADDRESS") + extra)
...
#    define LC_IDENTIFICATION_ADD_LEN(extra)                                \
                                          (STRLENs("LC_IDENTIFICATION") + extra)
...
#    define LC_MEASUREMENT_ADD_LEN(extra)                                   \
                                       (STRLENs("LC_MEASUREMENT") + extra)
...
#    define LC_PAPER_ADD_LEN(extra)      (STRLENs("LC_PAPER") + extra)
...
#    define LC_TELEPHONE_ADD_LEN(extra)    (STRLENs("LC_TELEPHONE") + extra)
...
#    define LC_NAME_ADD_LEN(extra)     (STRLENs("LC_NAME") + extra)
...
#    define LC_TOD_ADD_LEN(extra)      (STRLENs("LC_TOD") + extra)
 ...
 * setting multiple categories, CTYPE needs to match the other(s), and the way
 * the code is constructed, if we tried the other first, we might have to set
 * CTYPE twice. */
...
STATIC const Size_t lc_all_boiler_plate_length = 0
                                               + LC_CTYPE_ADD_LEN(2)
                                               + LC_NUMERIC_ADD_LEN(2)
                                               + LC_COLLATE_ADD_LEN(2)
                                               + LC_TIME_ADD_LEN(2)
                                               + LC_MESSAGES_ADD_LEN(2)
                                               + LC_MONETARY_ADD_LEN(2)
                                               + LC_ADDRESS_ADD_LEN(2)
                                               + LC_IDENTIFICATION_ADD_LEN(2)
                                               + LC_MEASUREMENT_ADD_LEN(2)
                                               + LC_PAPER_ADD_LEN(2)
                                               + LC_TELEPHONE_ADD_LEN(2)
                                               + LC_NAME_ADD_LEN(2)
                                               + LC_SYNTAX_ADD_LEN(2)
                                               + LC_TOD_ADD_LEN(2)
                                                 /* Doesn't include LC_ALL nor
                                                  * LC_UNKNOWN */
                                                ;
 ...
 * an individual locale, hence marks the lements here as not actually
 * initialized. */
STATIC
locale_category_index
map_LC_ALL_position_to_index[LOCALE_CATEGORIES_COUNT_] = { LC_ALL_INDEX_ };
...
# define get_category_index(cat) get_category_index_helper(cat, NULL,       \
                                                           __LINE__)
...
#    else
    ...
    char * individ_locales[LOCALE_CATEGORIES_COUNT_] = { NULL };
 ...
 *
 * 1) bool_setlocale_X  sets a given category's locale to a given value,
 *                      returning if it worked or not.
...
S_querylocale_emulate_safe_i(pTHX_ const locale_category_index cat_index,
                                   const line_t caller_line)
{
    assert(cat_index <= LOCALE_CATEGORIES_COUNT_ + 1);
 ...
 * structures.  The restore function restores to the locale on entry and
 * unlocks.  This is effectively a just-in-time locale setting scheme
 *
 * In order to accommodate the need for more than one category being used by a
 * function, this implements a stack.  The lock is called for each needed
 * category.  Since the locks are general semaphores, only the first call
 * results in an actual lock.  Each call thus changes the locale for its
 * category to the desired one, pushing onto the stack what it should be
 * restored to afterwards.  The paired unlock calls unwind the stack until the
 * final one causes the mutex to be released.  Most libc call require one or
 * two categories.
 *
 ...
 * when one thread is in the global locale.  By restoring, we leave code not under this scheme to have the global thread for itself.  There is a big caveat here, though.  That thread must run in a critical section
 * this implementation can work with code in another thread that operates in
 * the global locale.  The caveat is that that
 * code needs to be protected by mutexes.  This isn't the case with the other
 * thread-safe implementations. */

void
Perl_category_lock_i(pTHX_ locale_category_index cat_index,
                           const char * file,
                           const line_t caller_line)
{
    dSAVE_ERRNO;
...
#  ifdef USE_LOCALE_NUMERIC
        ...
        /* For all categories except LC_NUMERIC, PL_curlocales[] contains the
         * correct value to set to.  LC_NUMERIC is more complicated.  If it is
         * known that we should be in the C locale, use C.  If we are in the
         * underlying locale, use that.  Otherwise use the saved value */
...
void
Perl_category_unlock_i(pTHX_ locale_category_index cat_index,
                             const char * file,
                             const line_t caller_line)
{
...
STATIC const char *
S_querylocale_2008_i(pTHX_ const locale_category_index index, const line_t caller_line)
{
...
#    if defined(USE_QUERYLOCALE) && defined(_NL_LOCALE_NAME)
            ...
            /* glibc has this querylocale() equivalent, which experience so far
             * shows is thread-safe, along with its nl_langinfo_l() */
            retval = mortalized_pv_copy(
...
#  ifndef NEWLOCALE_HANDLES_DISPARATE_LC_ALL
             ...
             * locale to what it was at the time the function was called (saved
             * in 'entry_obj'), and return failure.
             * */
            ...
            for (locale_category_index i = 0; i < LC_ALL_INDEX_; i++) {
                ...
                for (locale_category_index j = 0; j < LC_ALL_INDEX_; j++) {
                                ...
                                const line_t caller_line)
{
     ...
     * use a series of name=value pairs, like
...
#    if ! defined(USE_LOCALE_NUMERIC)
    ...
    bool disparate = strNE(locales_list[0], "C");
...
#    else
    ...
    bool disparate = strNE(locales_list[1], "C");
...
S_find_locale_from_environment(pTHX_ const locale_category_index index)
{
     ...
     *
     *      Debian systems differ from this standard in having a LANGUAGE
                                ...
                                const line_t higher_caller_line)
{
     ...
     * this function to determine what it is.
     *
     ...
     *
     * If a line number is 0, it's stack (sort-of) frame is omitted; same if
     * its the same line number as the next higher caller. */
...
#  endif
     ...
     * libc may return a synonymous locale name instead of the input; and, if
     * there are locale categories that we are to ignore, any attempt to change
     * them away from "C" is overruled */
...
#  ifdef LC_ALL
        ...
        STDIZED_SETLOCALE_LOCK;
        lc_all_string = stdized_setlocale(LC_ALL, locale);
        STDIZED_SETLOCALE_UNLOCK;
