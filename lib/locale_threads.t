...
BEGIN {
    ...
    $has_ctype = locales_enabled('LC_CTYPE');
    ...
    if ($langinfo_codeset) {
        if ($ret{codeset}) {
            if (   $ret{codeset} ne $langinfo_codeset
                    ...
                    # If the name isn't a simple one, it won't be correct
                    # XXX what if positional notation?
                && $ret{locale_name} !~ /LC_CTYPE=/)
            {
...
sub add_trials($$;$)
{
  ...
  LOCALE:
    foreach my $locale (@locales) {
        ...
        if ($locale_name ne "C" && $locale_name ne "POSIX") {
            next if $category_name eq 'LC_COLLATE';
        }
...
SKIP: {
    ...
    for (my $i = 0; $i < @tests_by_thread; $i++) {
        ...
        push @cooked_tests, {
                              category_to_locale => \%category_to_locale,
                              tests => \@this_thread_cooked_tests,
                              thread => scalar @cooked_tests,
                            };
    }

    my $tests_expanded = Data::Dumper->Dump([ \@cooked_tests ],
                                            [ 'all_tests_ref' ]);
    ...
    fresh_perl_is("
        ...
        sub output_test_result {
            ...
            if (! \$utf8_matches) {
                if (utf8::is_utf8(\$copy)) {
                    ...
                    utf8::upgrade(\$copy);
                    print STDERR \" (result wasn't in UTF-8; converted for\",
                                 \" easier comparison)\";
                }
            }
            print STDERR \":\\n\";
        ...
        # Defines \\\$all_tests_ref, used below
        my $tests_expanded;
        ...
        sub setup {
                                    ...
                                    (sort keys \$categories_locales_ref->%*)
            {
                ...
                if (! setlocale(\$category_number, \$locale)) {
                    ...
                    print STDERR \"\\nthread \", threads->tid(),
                           \" setlocale(\$category_name (\$category_number),\",
                           \" \$locale) failed during setup\n\";
                    return 0;
                }
            }
